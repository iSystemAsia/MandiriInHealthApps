...
import 'package:mandiri_in_health/blocs/sales_activity/list_cubit.dart';
...
import 'package:mandiri_in_health/widgets/sales_activity/sales_activity_item.dart';

class SalesActivityList extends StatefulWidget {
  const SalesActivityList({Key? key}) : super(key: key);

  @override
  State<SalesActivityList> createState() {
    return _SalesActivityListState();
  }
}

class _SalesActivityListState extends State<SalesActivityList> {
  final _listCubit = SalesActivityListCubit();
  ...
  void _onScroll() {
    ...
    if (state is SalesActivityListSuccess &&
        state.canLoadMore &&
        !state.loadingMore) {
      ...
      case ProductViewType.list:
        if (item != null) {
          return Container(
            ...
            child: SalesActivityItem(
              onPressed: () {
                _onSalesActivityDetail(item);
              },
              item: item,
              type: _listMode,
            ),
  ...
  Widget _buildContent() {
    return BlocBuilder<SalesActivityListCubit, SalesActivityListState>(
