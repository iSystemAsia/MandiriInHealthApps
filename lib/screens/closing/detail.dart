...
import 'package:mandiri_in_health/blocs/closing/detail_cubit.dart';
...
class ClosingDetail extends StatefulWidget {
  const ClosingDetail({Key? key, required this.item}) : super(key: key);

  final ClosingModel item;

  @override
  State<ClosingDetail> createState() {
    return _ClosingDetailState();
  }
}

class _ClosingDetailState extends State<ClosingDetail> {
  final _scrollController = ScrollController();
  final _closingDetailCubit = ClosingDetailCubit();
  ...
  void initState() {
    ...
    _closingDetailCubit.onLoad(widget.item.Id);
  ...
  ///Build content UI
  Widget _buildContent(ClosingModel? closing) {
    ///Build UI loading
    List<Widget> action = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocProvider(
        create: (context) => _closingDetailCubit,
        child: BlocBuilder<ClosingDetailCubit, ClosingDetailState>(
          builder: (context, state) {
            ClosingModel? closing;
            if (state is ClosingDetailSuccess) {
              closing = state.closing;
            }
            return _buildContent(closing);
          },
        ),
      ),
    );
